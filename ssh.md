...
ssh-keygen -f work -C "<email-address>"  # create ssh key, -f == file-name
ssh-keygen -f mahesh -C "<email-address>"
...
### setup ssh to connect with remote server
```bash
# 1. create ssh key inside `~/.ssh` directory
ssh-keygen -f workspace   # create ssh key, -f == file-name

# 2. paste content of `workspace.pub` to `~/.ssh/authorized_keys` file on your remote-server
